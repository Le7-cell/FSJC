...
def spectrum_score_norm(y_pred, alpha=1.0):
    """
    计算归一化的光谱分数
    参数:
        y_pred: 预测的连续值结果 (N,)
    ...
    # 计算一阶差分
    diff = np.dot(D, y_pred)
    # 计算光谱分数
    score = np.sum(np.abs(diff)) / (N * alpha)
    # 归一化到[0,1]区间
    score_norm = 1.0 / (1.0 + score)
    
    return score_norm


def spectrum_score(y_pred):
    """
    计算光谱分数
    参数:
        y_pred: 预测的连续值结果 (N,)
