...
# 在数据加载和预处理部分，确保标签是单通道的
# 如果原始标签是多通道的，可能需要将它们合并为单通道
# 例如，可以使用以下代码将多类别标签转换为单通道：
if mask.shape[0] > 1:  # 如果标签是多通道的
    # 将多通道标签合并为单通道（例如，取最大值的索引）
    mask = mask.argmax(dim=0, keepdim=True)
